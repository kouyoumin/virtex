...
def main(_A: argparse.Namespace):
    ...
    train_size = len(dataset) - int(0.1*len(dataset))
    ...
    print('batch_size:', _C.OPTIM.BATCH_SIZE // dist.get_world_size(),'Workers:', _A.cpu_workers)
    ...
    # Load Self-Supervised Pretrained Visual Feature Extractor
    chkpt = torch.load('/home/kouyoumin/GeneralUNet/202107161046/best_model_valid_backbone.pth')
    sd = chkpt['state_dict']
    model.visual.cnn.conv1 = torch.nn.Conv2d(1, sd['conv1.weight'].shape[0], kernel_size=7, stride=2, padding=3, bias=False)
    model.visual.cnn.load_state_dict(sd, strict=False)
