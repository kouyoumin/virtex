...
parser.add_argument(
    "-s", "--vocab-size", type=int, default=3000,
    help="Total desired size of our vocabulary.",
)
parser.add_argument(
    "-o", "--output-prefix", default="virtex/data/datasets/vocab/mammo_3k",
...
def _read_captions(annotations_path: str) -> List[str]:
    ...
    captions: List[str] = []
    with open(annotations_path, 'rt', newline='', encoding='UTF-8-sig') as csvfile:
...
if __name__ == "__main__":
    ...
    captions: List[str] = _read_captions('/run/user/1000/gvfs/smb-share:server=172.16.16.254,share=projects/A3/VGHK/list.csv')
    captions += _read_captions('/run/user/1000/gvfs/smb-share:server=172.16.16.254,share=projects/A3/SPH/list.csv')
    captions += _read_captions('/run/user/1000/gvfs/smb-share:server=172.16.16.254,share=projects/A3/FEMH/list.csv')
